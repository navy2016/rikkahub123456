...
fun UIMessagePart.Video.encodeBase64(withPrefix: Boolean = true): Result<EncodedImage> = runCatching {
    android.util.Log.d("FileEncoder", "[VideoUpload] encodeBase64 called: url=$url, withPrefix=$withPrefix")
    when {
        this.url.startsWith("file://") -> {
            val filePath =
                this.url.toUri().path ?: throw IllegalArgumentException("Invalid file URI: ${this.url}")
            android.util.Log.d("FileEncoder", "[VideoUpload] Processing file: $filePath")
            val file = File(filePath)
            if (!file.exists()) {
                throw IllegalArgumentException("File does not exist: ${this.url}")
            }
            android.util.Log.d("FileEncoder", "[VideoUpload] File exists: size=${file.length()} bytes")
            val mimeType = guessVideoMimeType(file.name)
            android.util.Log.d("FileEncoder", "[VideoUpload] Detected mimeType: $mimeType")
            val encoded = file.encodeToBase64Streaming()
            android.util.Log.d("FileEncoder", "[VideoUpload] Base64 encoded: length=${encoded.length}")
            EncodedImage(
        ...
        this.url.startsWith("data:") -> {
            android.util.Log.d("FileEncoder", "[VideoUpload] Data URI detected")
            val mimeType = url.substringAfter("data:").substringBefore(";")
            EncodedImage(base64 = url, mimeType = mimeType)
        }
    ...
    android.util.Log.e("FileEncoder", "[VideoUpload] encodeBase64 failed: ${it.message}", it)
}
